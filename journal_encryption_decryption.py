...
def encrypt(plaintext: str, password: str) -> str:
    ...
    for char in plaintext:
        ...
        if char.isalpha():
            ...
            plaintext_value = ord(char.lower()) - base # numerical value of the plaintext character
            key_value = ord(password[key_index % key_len]) - base # shift value from password

            ciphertext_value = (plaintext_value + key_value) % 26 # numerical value of the ciphertext character by finding remainder
            encrypted_char = chr(ciphertext_value + base) # convert numerical value back to character
...
def main():
    ...
    text = read_text_file("journal.txt")

    encrypted_result = encrypt(text, password)
    decrypted_result = decrypt(encrypted_result, password)

    print(encrypted_result)
    print(decrypted_result)
    write_text_file("journal_encrypted.txt", encrypted_result)
